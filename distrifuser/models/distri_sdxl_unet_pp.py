...
class DistriSDXLUNetPP(BaseModel):  # for Patch Parallelism
    ...
    def forward(
        ...
        if distri_config.use_cuda_graph and not record:
            ...
            if static_inputs["sample"].shape != sample.shape:
                print("Mismatch detected:")
                print("Expected shape:", static_inputs["sample"].shape)
                print("Actual shape:", sample.shape)
            ...
            if distri_config.world_size == 1:
                ...
                if self.distri_config.rank == 0 and self.distri_config.verbose:
                    print(f'Completed forward. before all gather')
